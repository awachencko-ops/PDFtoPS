...
- [ ] Вынести логику запуска Ghostscript в отдельный сервис `GhostscriptRunner`.
- [ ] Добавить structured-логирование в файл (`logs/yyyyMMdd.log`).
- [ ] Добавить таймауты и ретраи (уже начато в текущем изменении).
- [ ] Добавить коды ошибок и понятные сообщения в UI.

### Этап B — Реальная многопоточность
- [ ] Перевести конвертацию на `Task.Run` + `CancellationToken`.
- [ ] Ограничить параллелизм через `SemaphoreSlim` (например, 2 потока по умолчанию).
- [ ] Добавить безопасное обновление UI через `Invoke`/`BeginInvoke`.
- [ ] Реализовать кнопку “Отмена” для пачки.

### Этап C — Устранение архитектурных конфликтов
- [ ] Разделить UI и доменную логику (WinForms слой / сервисы / модели).
- [ ] Вынести профили и параметры в JSON-конфиг.
- [ ] Покрыть критическую логику unit/integration тестами.
- [ ] Добавить предварительную валидацию PDF (битые/защищенные документы).

### Этап D — Надежность поставки
- [ ] Добавить health-check Ghostscript при старте приложения.
- [ ] Поддержать portable-режим с локальным `gs` рядом с exe.
- [ ] Зафиксировать поддерживаемые версии Ghostscript в документации.
